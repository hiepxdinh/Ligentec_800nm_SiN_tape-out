...
from All_pass_ring_taper import All_pass_ring_Exspot, All_pass_ring_Exspot_2
from Add_drop_ring_taper import Add_drop_ring_Exspot
...
for i, gap in enumerate(coupler_gap):
    ...
    chip_elements.append(i3.SRef(reference=ring, position=(3095 + 200*i, 3615), transformation=i3.Rotation(rotation=90)))
...
# h_separation = 550.0
# v_separation = 35.0
# separation = (0, 100, 100, 100, 100)
# offset = 250
# out_taper_position = 150
# gap_list = [2.05]
# for i, gap in enumerate(separation):
#     name = "All_Pass_Ring_{}".format(radius)
#     ring = All_pass_ring_Exspot_2(ring_position_x=-i*h_separation+150, ring_position_y=(1-i)*v_separation, output_offset=offset, out_taper_position = out_taper_position)
#     ring_lv = ring.Layout(ring_radius=radius)
#     # ring_lv.visualize(annotate=True)
#     chip_elements.append(i3.SRef(reference=ring, position=(6595, 1500+1600), transformation=i3.Rotation(rotation=90)))
...
gap_list = [2.05]
# for i, gap in enumerate(coupler_gap):
ring = Add_drop_ring_Exspot(ring_position_x=0, ring_position_y=0, output_offset=0)
ring_lv = ring.Layout(ring_radius=radius)
# ring_lv.visualize(annotate=True)
chip_elements.append(i3.SRef(reference=ring, position=(7895 + 200*1, 3615), transformation=i3.Rotation(rotation=90)))
...
for i, width in enumerate(waveguide_width):
    ...
    chip_elements.append(i3.SRef(reference=waveguide_test, position=(-3000+3625, 1800 + i*separation - 7.5*i)))
...
for i, gap in enumerate(coupler_gap):
    ...
    chip_elements.append(i3.SRef(reference=bg_2_1_lv, position=(-3000+4725, 50*(i+1) - 7.5*i)))
...
for i, gap in enumerate(coupler_gap):
    ...
    chip_elements.append(i3.SRef(reference=bg_3_1_lv, position=(-3000+5025, 250 + 50*(i+1) - 7.5*i)))
...
for i, gap in enumerate(coupler_gap):
    ...
    chip_elements.append(i3.SRef(reference=bg_5_1_lv, position=(-3000+7010, 500 + 50*(i+1) - 7.5*i)))
...
for i, gap in enumerate(coupler_gap):
    ...
    chip_elements.append(i3.SRef(reference=bg_6_1_lv, position=(-3000+7010-590, 750 + 50*(i+1) - 7.5*i)))
...
for i, gap in enumerate(coupler_gap):
    ...
    chip_elements.append(i3.SRef(reference=bg_8_1_lv, position=(-3000+7010-100, 1000 + 50*(i+1) - 7.5*i)))
...
for i, gap in enumerate(coupler_gap):
    ...
    chip_elements.append(i3.SRef(reference=bg_10_1_lv, position=(-3000+7010-1250, 1250 + 50*(i+1) - 7.5*i)))
...
for i, gap in enumerate(coupler_gap):
    ...
    chip_elements.append(i3.SRef(reference=bg_10_1_lv, position=(-3000+7010-1250, 1500 + 50*(i+1) - 7.5*i)))
...
for i, gap in enumerate(coupler_gap):
    ...
    chip_elements.append(i3.SRef(reference=bg_1_1_lv, position=(0+ 50*(i+1) - 7.5*i, 3100-255+77), transformation=i3.Rotation(rotation=90)))
...
for i, gap in enumerate(coupler_gap):
    ...
    chip_elements.append(i3.SRef(reference=bg_4_1_lv, position=(250+ 50*(i+1) - 7.5*i, 3100-255+77-113), transformation=i3.Rotation(rotation=90)))
...
for i, gap in enumerate(coupler_gap):
    ...
    chip_elements.append(i3.SRef(reference=bg_7_1_lv, position=(500+ 50*(i+1) - 7.5*i, 3100-255+77+23), transformation=i3.Rotation(rotation=90)))
